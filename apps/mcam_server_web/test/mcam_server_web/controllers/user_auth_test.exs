defmodule McamServerWeb.UserAuthTest do
  ...
  setup %{conn: conn} = ctx do
    ...
    %{user: user_fixture(%{confirm?: !ctx[:skip_confirm?]}), conn: conn}
  end
  ...
  describe "require_authenticated_user/2" do
    ...
    @tag skip_confirm?: true
    test "does not require confirmation", %{conn: conn, user: user} do
      conn =
        conn
  ...
  describe "require confirmed user" do
    ...
    @tag skip_confirm?: true
    test "redirects to confirm page if not confirmed", %{conn: conn, user: user} do
      conn =
        conn
        |> assign(:current_user, user)
        |> fetch_flash()
      ...
      assert get_flash(conn, :error) =~ "confirm your email"
    end
  end
end
